...
def search_legacy(
    ...
    """
    Legacy unfiltered search endpoint (kept for backward compatibility).
    
    Note: Prefer using search_all() or specific type searches for better results.
    ...
    cache_key: CacheKey = (query, f"legacy_{filter or 'all'}", limit)
    
    if use_cache:
        cached = _get_from_cache(cache_key)
        if cached is not None:
            logger.debug(f"Cache HIT for search_legacy: {query}")
            return cached
    
    logger.debug(f"Cache MISS for search_legacy: {query}")
    
    # Determine allowed types from filter
    allowed_types = None
    if filter:
        filter_lower = filter.lower()
        if filter_lower in ("songs", "song", "track", "tracks"):
    ...
    try:
        ...
        logger.exception(f"Adapter search failed for query '{query}'")
        raise Exception(f"Search service unavailable: {str(e)}")
    
    # Ensure consistency
    normalized_results = normalizers.ensure_search_result_consistency(results)
