        ...
        {artists.map((artist) => {
          ...
          return (
            <button
              ...
              title={isFollowed ? t('charts.followedArtist') : undefined}
            >
              {/* Artist Image Circle */}
              <div className="relative w-16 h-16">
                ...
                {/* Followed Badge - Top Right */}
                {isFollowed && (
                  <div className="absolute -top-1 -right-1 z-10 w-5 h-5 rounded-full bg-green-500 flex items-center justify-center shadow-lg border-2 border-white dark:border-zinc-900">
                    <span className="text-[10px] font-bold text-white">✓</span>
                  </div>
                )}

                {/* Artist Image */}
                <div className={`w-full h-full rounded-full overflow-hidden bg-slate-200 dark:bg-white/10 border-2 transition-all group-hover:border-blue-500 dark:group-hover:border-red-500 ${
                  isFollowed
